...
_TIMEOUT = 10  # seconds — fail-fast on hung dev servers
...
def test_full_flow():
    ...
    r = requests.get(f"{BASE_URL}/v2/balance", headers=sender_headers, timeout=_TIMEOUT)
    ...
    r = requests.post(f"{BASE_URL}/v2/balance/deposit",
                      json={"amount": 10.0},
                      headers=sender_headers, timeout=_TIMEOUT)
    assert r.status_code == 200
    ...
    r = requests.get(f"{BASE_URL}/v2/balance", headers=recipient_headers, timeout=_TIMEOUT)
    assert r.status_code == 200
    ...
    r = requests.post(f"{BASE_URL}/v2/escrow/create", json={}, timeout=_TIMEOUT)
    assert r.status_code in [401, 501]
    print("[OK] Escrow endpoint correctly disabled")

    r = requests.post(f"{BASE_URL}/v2/payments/send", json={}, timeout=_TIMEOUT)
    assert r.status_code in [401, 501]
