...
class AWSKMSManager(HSMBackend):
        ...
        try:
            response = self.client.create_key(
                ...
                Tags=[
                    ...
                    {'TagKey': 'Project', 'TagValue': 'StealthPay'}
                ]
            )
            ...
            key_alias = alias or f"alias/stealthpay-mpc-{party_id}"
            try:
    ...
    def store_key_share(
        ...
        return KeyShareHSM(
            ...
            alias=alias or f"alias/stealthpay-mpc-{party_id}",
            party_id=str(party_id),
            backend='aws_kms',
            metadata={
    ...
    def _get_or_create_key(self, party_id: str) -> str:
        ...
        alias = f"alias/stealthpay-mpc-{party_id}"
        try:
