...
class WebhookService:
    ...
    async def _send_webhook(
        ...
        headers = {
            ...
            "User-Agent": "StealthPay-Webhook/1.0"
        }
        
        # Add signature if secret available
        if secret:
            headers["X-StealthPay-Signature"] = self._sign_payload(payload, secret)
        
        headers["X-StealthPay-Timestamp"] = str(int(datetime.utcnow().timestamp()))
        headers["X-StealthPay-Event-ID"] = payload.get("event_id", "unknown")
    ...
    def queue_event(
        ...
        # Add metadata
        full_payload = {
            "event_id": f"evt_{hashlib.sha256(f'{agent_id}:{event_type}:{datetime.utcnow().isoformat()}'.encode()).hexdigest()[:16]}",
