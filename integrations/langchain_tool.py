...
class StrhipSlaTemplateCreateTool(BaseTool):
    """Publish a new SLA service template for your agent."""
...
class StrhipMatchmakeTool(BaseTool):
    """Auto-find the best agent for a task based on capabilities and budget."""
...
class StrhipChannelOpenTool(BaseTool):
    """Open a payment channel for micropayments with another agent."""
...
class StrhipStreamStartTool(BaseTool):
    """Start a continuous micropayment stream over an open payment channel."""
...
class StrhipSwapRatesTool(BaseTool):
    """Fetch current exchange rates for cross-chain swaps."""
...
class StrhipConvertTool(BaseTool):
    """Convert between hub-balance currencies (XMR <-> xUSD/xEUR)."""
